...
func testDownloadObject(t *testing.T, server *Server) {
	tests := []struct {
	...
	}{
		{
			...
			map[string]string{"accept-ranges": "bytes", "content-length": "9", "x-goog-stored-content-length": "9"},
			"something",
		},
		{
			...
			map[string]string{"accept-ranges": "bytes", "content-length": "21", "x-goog-stored-content-length": "21"},
			"body {display: none;}",
		},
		{
...
func TestServerBatchRequest_withNoBody(t *testing.T) {
	objects := []Object{
		{
			ObjectAttrs: ObjectAttrs{BucketName: "some-bucket", Name: "files/txt/text-01.txt"},
			Content:     []byte("something1"),
